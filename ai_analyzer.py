...
RESUME_DIALOG_PROMPT = """Ты анализируешь, просит ли пользователь снять паузу общения и продолжить диалог после конфликта.

Верни should_resume=true, если в последнем сообщении есть прямое ИЛИ неявное извинение/примирение:
- «извини/прости/сорян/виноват/погорячился»
- «давай нормально», «мир?», «не злись», «ладно, был не прав»
- мягкая форма примирения даже без слова «извини».

Верни should_resume=false, если это:
- продолжение оскорблений/агрессии,
- нейтральная реплика без примирения,
- сарказм без реального запроса продолжить общение.
