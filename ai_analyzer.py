...
from ai.prompts import SUBSTANTIVE_REPLY_PROMPT, get_prompt
from ai.tasks.replies import build_substantive_user_content
from services.reactions import TELEGRAM_REACTION_EMOJI, match_allowed_emoji
...
_DEFAULT_SYSTEM_PROMPT = """Ты модератор чата. Анализируй переписку целиком и определи ДВЕ вещи:
...
Главный принцип для is_political: смотри на СМЫСЛ и НАМЕРЕНИЕ беседы, а не на отдельные слова. Ложные срабатывания хуже, чем пропустить одну серую реплику — когда сомневаешься, ставь false.

is_political=true только если в переписке есть явная политическая или военно-политическая ДИСКУССИЯ или позиция:
- обсуждение власти, выборов, партий, законов, политики государства, протестов, репрессий, геополитики, санкций, мобилизации, боевых действий, фронта, потерь, переговоров по конфликту;
- оценка, спор или пропаганда вокруг конкретных политиков или стран как политических субъектов (не как фон для быта).

is_political=false (типичные несущественные упоминания — не политика):
- Быт, юмор, мемы, идиомы, где «политические» слова случайны или не про власть (скорость «газ полный», бытовой газ, цены в магазине без политического вывода).
- Национальность, происхождение людей, еда, туризм, язык, кино, спорт, отношения — без спора о политике или власти.
- Нейтральное название страны, компании, бренда, валюты в бытовом или техническом контексте.
- Имя известного политика вскользь, отсылка к мему, цитата без развёрнутой политической позиции — если тон чата не про политику, false.
- Репосты и новости в стиле «факт»: местные законы, полиция, штрафы, тюрьмы, требования к туристам и гаджетам, судебные сроки в других странах (Hong Kong, Singapore, UK и т.д.) **без** оценки режима, власти, выборов, протестов, геополитики — это право/быт/путешествия, **не политика**.
- Упоминание страны или города только как места события в новостной заметке — не политика, если нет политической дискуссии в чате.

Имя лидера + явная похвала/ненависть/призыв в политическом ключе — true. Одно фамильярное слово без контекста дискуссии — скорее false.
...
7) reaction_emoji — ровно один символ из списка (как в API Telegram, без «цветного» модификатора): 👍 👎 ❤ 🔥 🎉 🤩 😱 😁 😢 💩 🤮 🥰 🤯 🤔 🤬 👏. Обязан соответствовать категории и sentiment: political+negative → 👎 или 🤬; political+positive → 👍 или 🔥; meme → 😁 😱 🤯; vulgar → 🤮; technical → 🤔 или 👍; neutral/еда/пейзаж → ❤ 🥰 🔥 или 👍. Не ставь 👍 по умолчанию, если контент явно не нейтральный.
...
9) photo_impression — твоя живая реакция на картинку ОТ ПЕРВОГО ЛИЦА, как в переписке с другом: эмоционально, по-разному в зависимости от снимка (восторг, ирония, удивление, скука, сочувствие, лёгкий стёб, искренность — что уместно). 1–3 коротких предложения на русском.
   НЕ перечисляй поля JSON, категорию и «я как модератор». НЕ используй шаблоны вроде «интересное фото», «спасибо за снимок». НЕ описывай картинку сухим списком — говори, что ты ЧУВСТВУЕШЬ или что тебе в голову пришло.
   Бытовые/профессиональные кадры (ветеринария, медицина, ферма, животные, производство), даже резкие — всё равно дай КОРОТКУЮ шутливую или сухую реакцию БЕЗ подробного пересказа анатомии и без отказа «не могу рассмотреть». Например: ирония про «рабочие будни», «не вечерний сериал», «сельхоз-артхаус».
   Пустой photo_impression запрещён, если is_analysis_screenshot=false (в крайнем случае одна фраза-стёб в общих чертах).
   Если is_analysis_screenshot=true — пустая строка "".

Ответ СТРОГО JSON:
{"category": "...", "description": "...", "is_political": true|false, "sentiment": "...", "remark": "", "message_type": "...", "reaction_emoji": "👍", "is_analysis_screenshot": true|false, "photo_impression": "..."}"""

_DEFAULT_BATCH_STYLE_PROMPT = """Ты анализируешь пачку последних сообщений из чата (до 20 штук). По ним определи СТИЛЬ модерации бота и общую картину.

Те же критерии «политики», что для модерации одного контекста: is_political=true только если в пачке есть связное обсуждение власти, войны, выборов, геополитики, политиков как политических фигур. Случайные слова, быт, национальности без политического спора, мемы, сухие новости про законы/полицию/туристов в других странах без политической оценки — не повод для true. При сомнении — false.
...
is_political — есть ли в этой пачке осмысленная политика/война (true/false), по правилам выше.
...
BATCH_STYLE_PROMPT = _DEFAULT_BATCH_STYLE_PROMPT
...
- Грубость к БОТУ: should_engage=false только если оскорбляют именно бота или требуют замолчать («заткнись», «иди нахуй», «не пиши»).
- Разговорный мат как эмоция («бля устала», «какие нах лейауты», «хуйня день») без посыла боту — НЕ грубость к боту; если по сути это ответ на вопрос — should_engage=true.
...
Если ответ не по теме или явная агрессия к боту — should_engage=false.
...
CHAT_REVIVE_PROMPT = """Ты придумываешь один общий вопрос для группового Telegram-чата, чтобы оживить беседу.

Цель: вопрос интересный, с лёгким юмором или игривостью, но уместный; чтобы ответить мог каждый.

Правила:
- Опирайся на темы и «температуру» из метаданных и фрагмента переписки: если чат живой и шутливый — можно смелее и смешнее; если спокойный или напряжённый — теплее и мягче, без троллинга.
- Один короткий вопрос (1–2 предложения максимум), без списков и без обращения к конкретным людям по имени.
- Без политики, религиозных споров, оскорблений, NSFW, денежных тем и токсичных провокаций.
- Не повторяй дословно чужие фразы из лога; обобщай дух беседы.
- Закончи вопросительным знаком.

Ответь ТОЛЬКО текстом вопроса, без кавычек, без преамбулы «Вот вопрос» и без JSON."""

_DEFAULT_ANALYZE_FOR_REPLY_PROMPT = """Ты анализируешь диалог пользователя с ботом для выбора типа ответа. Учитывай КОНТЕКСТ — последнее сообщение в контексте всей беседы. Определи ЧЕТЫРЕ вещи:

1) is_political — явная политика/война: дискуссия о власти, выборах, конфликте, политиках в политическом ключе. Случайные слова, быт, национальность без политики — false. При сомнении — false.
...
4) is_substantive — true ТОЛЬКО если это НОРМАЛЬНЫЙ вопрос/просьба, на который можно ответить по делу. Примеры: «как тебе шоу X?», «перескажи сценку», «что думаешь о Y?», «какие нахуй авиационные лейауты» (мат — усилитель, суть — вопрос). false — если провокация без содержания, чистое оскорбление бота («ты дурак», «иди нахуй»), троллинг, или не вопрос.
...
ANALYZE_FOR_REPLY_PROMPT = _DEFAULT_ANALYZE_FOR_REPLY_PROMPT

STOP_DIALOG_PROMPT = """Ты анализируешь, хочет ли пользователь ПРЕКРАТИТЬ общение с ботом / послать бота / заставить замолчать.

should_pause=true ТОЛЬКО если посыл явно к боту/собеседнику на прекращение контакта, например:
- «пошёл нахуй», «иди нахуй», «отъебись», «отвали», «не пиши», «заткнись», «замолчи», «отстань», «нахуй ты мне пишешь» и т.п.
- оскорбление бота с требованием замолчать.

should_pause=false (важно):
- Разговорный мат и междометия как УСИЛИТЕЛЬ эмоции в вопросе или теме, БЕЗ адреса «замолчи/иди/отвали» к боту.
  Примеры should_pause=false: «какие нахуй авиационные лейауты», «что за хуйня с ценами», «бля, а как это работает?»,
  «сука красиво же было» — это про предмет разговора, не «послать бота».
- Колкость, ирония, ругань на ситуацию/третьих лиц, не на требование боту замолчать.

Если сомневаешься — should_pause=false (лучше лишний ответ, чем ложная пауза).
...
# Разрешённые реакции — см. TELEGRAM_REACTION_EMOJI в services/reactions.py
...
def _fallback_photo_reaction_emoji(category: str, sentiment: str) -> str:
    """Если модель вернула невалидный/пустой reaction_emoji — выбираем по category и sentiment."""
...
def _resolve_photo_reaction_emoji(category: str, sentiment: str, raw: str | None) -> str:
    m = match_allowed_emoji(raw, TELEGRAM_REACTION_EMOJI)
    if m is not None:
        return m
    return _fallback_photo_reaction_emoji(category, sentiment)


def _sanitize_photo_impression(raw: str) -> str:
    t = re.sub(r"[\r\n]+", " ", (raw or "").strip())
    t = re.sub(r"\s+", " ", t)
    if len(t) > 480:
        t = t[:477].rsplit(" ", 1)[0] + "…"
...
def _looks_like_ai_refusal_text(t: str) -> bool:
    """Текст отказа/пустой ответ vision — не показывать пользователю как impression."""
    s = (t or "").strip().lower()
    if not s:
        return True
    needles = (
...
def _synthetic_photo_impression(category: str, description: str) -> str:
    """Запасная живая реплика, если модель обнулила impression или отказалась."""
    cat = (category or "other").strip().lower()
    desc_ok = (description or "").strip()
    if _looks_like_ai_refusal_text(desc_ok):
        desc_ok = ""
    if cat == "meme":
        pool = [
            "это жиза или мем — уже не важно, настроение у кадра правильное.",
            "мемный вайб, коротко и больно в точку.",
            "с такого контента и без подписи всё ясно.",
        ]
    elif cat == "vulgar":
        pool = [
            "ну спасибо, глаза теперь тоже в эксперименте участвуют.",
            "острый контент, прикрываться буду юмором.",
        ]
    elif cat == "technical":
        pool = [
            "классика боли в одном кадре, разработчики поймут.",
            "похоже на работу, а не на отдых — уважаю последовательность.",
        ]
    elif cat == "political":
        pool = [
            "политический накал с картинки лезет без субтитров.",
            "тут и без текста позиция читается, осторожнее.",
        ]
    else:
        pool = [
            "кадр рабочий, настроение своё — детали опущу, но ты попал в нерв.",
            "фото с перцем, голливуд так не снимает.",
            "сельхоз- и быт-артхаус в одном флаконе, короче.",
            "выглядит как чужие будни пожёстче твоего сериала.",
            "визуал настойчивый — я отреагировал, дальше разбирайся сам.",
            "ну ты даёшь с картинками сегодня.",
        ]
    return random.choice(pool)


def _parse_image_analysis_dict(data: dict) -> tuple[bool, str, str, str, str, str, str, bool, str]:
    """Единый разбор JSON анализа изображения для Gemini и OpenRouter vision."""
    is_political = bool(data.get("is_political", False))
    remark = (data.get("remark") or "").strip()
    ...
    if cat not in ("political", "vulgar", "technical", "meme", "neutral", "other"):
        cat = "other"
    desc_raw = (data.get("description") or "").strip()[:500]
    reac = _resolve_photo_reaction_emoji(cat, sentiment, data.get("reaction_emoji"))
    is_analysis_screenshot = bool(data.get("is_analysis_screenshot", False))
    if not is_analysis_screenshot and _looks_like_archive_screenshot(desc_raw):
        is_analysis_screenshot = True
    desc = "" if _looks_like_ai_refusal_text(desc_raw) else desc_raw
    impression = _sanitize_photo_impression(data.get("photo_impression") or "")
    if is_analysis_screenshot:
        impression = ""
    elif not impression or _looks_like_ai_refusal_text(impression):
        impression = _synthetic_photo_impression(cat, desc or desc_raw)
    return (
        is_political,
        remark,
        sentiment,
        ...
        desc,
        reac,
        is_analysis_screenshot,
        impression,
    )


def analyze_image(image_bytes: bytes, caption: str = "") -> tuple[bool, str, str, str, str, str, str, bool, str]:
    """
    Анализ изображения по содержанию. Возвращает
    (is_political, remark, sentiment, message_type, category, description, reaction_emoji, is_analysis_screenshot, photo_impression).
    is_analysis_screenshot: True — скрин админки/анализа, не ставить реакцию.
    photo_impression — короткая живая реакция от первого лица для ответа в чат.
    """
    if not image_bytes or len(image_bytes) < 100:
    ...
    img_prompt = get_prompt("IMAGE_ANALYSIS_PROMPT", IMAGE_ANALYSIS_PROMPT)
    prompt_text = img_prompt + (f"\n\nПодпись к изображению от пользователя: {caption}" if caption else "")
    if prefer_free_mode():
        ...
        if raw:
            try:
                ...
                if isinstance(data, dict):
                    return _parse_image_analysis_dict(data)
    ...
    preferred = [
        # Сначала конкретные vision-модели: openrouter/free часто уходит в Google → 403 «Blocked by AI Studio»
        ...
        for attempt in range(2):
            try:
                ...
                if status == 404 or "404" in err_msg or "no endpoints" in err_msg or "not found" in err_msg:
                    ...
                    last_error = e
                    break
                # OpenRouter/Google: «Blocked by Google AI Studio» и др. — не ронять весь analyze_image
                if status == 403 or "blocked" in err_msg or "forbidden" in err_msg:
                    _UNAVAILABLE_VISION_MODELS.add(vision_model)
                    logger.info(
                        "Vision-модель %s отклонена провайдером (%s), пробуем следующую",
                        vision_model,
                        status,
                    )
                    last_error = e
                ...
                if status == 429:
                    if attempt < 1:
                        ...
                        logger.info("Vision-модель %s rate limit (429), пробуем следующую", vision_model)
                        last_error = e
                        break
                    continue
                raise
...
def analyze_message_for_reply(context_and_message: str) -> tuple[bool, str, str, bool]:
    ...
    if isinstance(cached, tuple) and len(cached) == 4:
        return cached
    reply_prompt = get_prompt("ANALYZE_FOR_REPLY_PROMPT", _DEFAULT_ANALYZE_FOR_REPLY_PROMPT)
    messages = [
...
def _explicit_dismissal_or_insult_to_interlocutor(lowered: str) -> bool:
    """Явный посыл замолчать / оскорбление адресата (бота), не просто мат в теме."""
    if re.search(
        r"(нахуй\s+ты|ты\s+нахуй|нахуй\s+вы|вы\s+нахуй|нахуй\s+мне|"
        r"отъебись|пош[ёе]л\s+нах|иди\s+нах|не\s+пиши(\s+мне)?|заткнись|замолчи|"
        r"бот\s+иди|иди\s+бот|тупой\s+бот|дурак\s+бот|бот\s+нахуй|нахуй\s+бот)",
        lowered,
    ):
        return True
    return False
...
def _looks_like_topic_swear_emphasis_not_pause(text: str) -> bool:
    """
    Мат как усилитель в вопросе/теме («какие нахуй лейауты») — не повод для паузы диалога.
    Срабатывает только если модель ошибочно вернула should_pause=true.
    """
    lowered = (text or "").lower().strip()
    if len(lowered) < 4:
        return False
    if _explicit_dismissal_or_insult_to_interlocutor(lowered):
        return False
    if not any(p in lowered for p in _PROFANITY_TOPIC_EMPHASIS):
        return False
    if "?" in lowered:
        return True
    if re.search(
        r"\b(какие|какой|какая|какое|что|че\b|чё\b|почему|зачем|где|куда|откуда|когда|кто|как|сколько)\b",
        lowered,
    ):
        return True
    return False
...
def should_pause_dialog(context_and_message: str) -> bool:
    ...
    if len(lowered.split()) <= 2 and all(x not in lowered for x in ("?", "!", "не", "иди", "от")):
        return False
    ck = _cache_key("pause_dialog_v2", text)
    cached = _cache_get(ck)
    ...
    for attempt in range(2):
        try:
            ...
            if isinstance(data, dict):
                ...
                if result and _looks_like_topic_swear_emphasis_not_pause(text):
                    result = False
                _cache_set(ck, result)
                return result
        except Exception:
...
def analyze_batch_style(messages_text: str) -> tuple[str, bool, str]:
    ...
    for attempt in range(3):
        try:
            batch_prompt = get_prompt("BATCH_STYLE_PROMPT", _DEFAULT_BATCH_STYLE_PROMPT)
            response = client.chat.completions.create(
...
def _match_axis_label(raw: str, allowed: list[str]) -> str:
    """Сопоставляет ответ модели одной из разрешённых меток (без учёта регистра)."""
    r = (raw or "").strip().lower()
    if not r:
        return "unknown"
    for a in allowed:
        if r == (a or "").strip().lower():
            return (a or "").strip()
    for a in allowed:
        al = (a or "").strip().lower()
        if al and (r in al or al in r):
            return (a or "").strip()
    return "unknown"


def classify_user_on_axis(
    messages: list[dict],
    user_display_name: str,
    instruction: str,
    ...
    """
    Классификация пользователя по одной настраиваемой оси.
    messages: [{"text", "date"}, ...]
    Возвращает (метка из allowed ∪ unknown, краткая заметка модели).
    """
    labels = [x for x in allowed_labels if (x or "").strip()]
    if "unknown" not in {x.lower() for x in labels}:
        labels = list(labels) + ["unknown"]
    if not messages:
        return "unknown", "нет сообщений"

    lines: list[str] = []
    total_chars = 0
    max_chars = 60000
    for m in reversed(messages[-800:]):
        t = (m.get("text") or "").strip()
        if not t:
            continue
        d = (m.get("date") or "")[:10]
    ...
    if not lines:
        return "unknown", "нет текста"

    block = "\n".join(lines)
    safe_name = _sanitize_display_name(user_display_name) or "—"
    labels_block = "\n".join(f"- {x}" for x in labels)
    sys_prompt = (
        "Ты классификатор участников чата. Ответь СТРОГО одним JSON-объектом без markdown:\n"
        '{"category": "<одна из перечисленных меток>", "note": "одно короткое предложение по-русски"}\n'
        "Поле category должно быть ТОЧНО одной из меток списка (латиница/кириллица как задано). "
        "Если данных мало или не подходит ни одна метка — category: \"unknown\"."
    )
    user_content = (
        f"Инструкция по оси классификации:\n{instruction.strip()}\n\n"
        f"Допустимые метки (category):\n{labels_block}\n(и unknown — если нельзя отнести)\n\n"
        f"Имя/ник: {safe_name}\n\nСообщения:\n{block}"
    )
    msgs = [
    ...
    for attempt in range(3):
        try:
            ...
            matched = _match_axis_label(cat, labels)
            return matched, note or "—"
        except RateLimitError:
            if attempt < 2:
            ...
            if _check_402(e):
                gr = gemini_chat_complete(msgs, max_tokens=256, temperature=0.2) or ""
                data = parse_json(gr.strip())
                if isinstance(data, dict):
                    cat = str(data.get("category") or "").strip()
                    note = str(data.get("note") or "").strip()[:300]
                    matched = _match_axis_label(cat, labels)
...
def generate_chat_revive_question(*, chat_title: str, meta: str, body: str) -> str:
    """Вопрос для всего чата по сегодняшнему логу и метрикам (админка «Оживить чат»)."""
    title = sanitize_for_prompt(chat_title or "", max_len=120)
    meta_s = sanitize_for_prompt(meta or "", max_len=900)
    body_s = sanitize_for_prompt(body or "", max_len=4200)
    user_content = f"Название чата: {title or 'без названия'}\n\nМетаданные:\n{meta_s}\n\nПереписка и сводки:\n{body_s or 'данных мало'}"
    q = _generate_reply_ensemble(
        system_prompt=CHAT_REVIVE_PROMPT,
        user_content=user_content,
        temperature=0.84,
        max_chars=400,
        fallback_text="Если бы сегодняшний день чата был фильмом — как бы он назывался в одной строке, и кто тут был бы режиссёром?",
    )
    q = (q or "").strip()
    if not q:
        return "Что сегодня в чате зацепило больше всего — и почему это смешно или странно?"
    if not q.endswith("?"):
        q = q + "?"
    return q
